...
- **Synthetic seed data** with both legacy PoC and generated datasets (small and large configs) that keep the same operational schema.
...
- **Streamlit app** with predefined demo questions, free-text entry, Claude-based intent routing, and a small subgraph view (tables + simple diagram).
...
|------|---------------------|
| **Richer synthetic data** | Extend the configurable generator with additional motifs while preserving schema compatibility and hidden eval separation. |
...
| **Improved natural language routing** | Tighten intent confidence handling and structured decomposition for multi-step investigations. |
