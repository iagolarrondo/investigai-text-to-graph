...
- **Streamlit app** — free-text investigations via an LLM **tool planner → judge → synthesis** loop on `query_graph` tools; **session-scoped memory** (same tab, not persisted) for follow-up rewrite/clarify, optional entity picks, **HTML session export**, optional **NetworkX vs Neo4j compare** (does not write to session/export), and **eval / backend-comparison** sidebar pages. Copy **`env.template`** → **`.env`** for keys (see README). Hosted or local LLM keys are required for investigations.
...
- **Optional router** (`src/llm/router.py`) is rule/Claude-based for **non–main-app** experiments; the **primary** investigation path is still the hosted/local **tool planner** (not a separate fraud NLU product).
