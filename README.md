...
- Open **`src/app/app.py`**, where **session-scoped memory** can rewrite or clarify follow-ups, then an LLM runs a **tool-planner** (with graph tools), a **coverage judge** reads the **full** tool trace (outer loop until satisfied), then **synthesis** produces the **Answer** and a **graph focus** for the summary pyvis view. You can **export** an HTML session report or **clear** memory from the same page. By default (**`INVESTIGATION_MERGE_JUDGE_SYNTHESIS`**, see **`env.template`**) the judge may return the answer in the same JSON when satisfied, skipping a separate synthesis call. Configure **`INVESTIGATION_LLM`** and API keys as in **§4** (Gemini, Anthropic, or local Ollama with a **tool-capable** model, for example `ollama pull llama3.1`).
...
#### Session-scoped memory (same tab only; not persisted)
...
- **Before** pre-run entity resolution and the planner: **`resolve_question_with_session_memory`** (`src/session/context_resolver.py`) may **pass through**, **rewrite** (e.g. pronouns or “that claim” → concrete node ids from recent turns + rolling referents), or **clarify** (block with a short prompt until the user runs as typed or cancels). Deterministic rules run first; an optional **LLM fallback** runs for harder contextual follow-ups when **`SESSION_MEMORY_LLM_REWRITE`** is on (see **`env.template`**).
...
- **Troubleshoot connectivity:** from the project root, run **`PYTHONPATH=. python -m src.graph_store.neo4j_diagnose`**. It checks `verify_connectivity`, then counts **`:Entity`** nodes (must be non-zero after **`PYTHONPATH=. python -m src.graph_store.sync_processed --clear`**). Common mistakes: **`NEO4J_URI`** missing the full Aura host (must be like **`neo4j+s://<instance-id>.databases.neo4j.io`**), or **`NEO4J_DATABASE`** set to the numeric instance id — use the **database name** **`neo4j`** (or leave **`NEO4J_DATABASE`** unset). On the **Investigate** page, pick **Neo4j** as the graph backend when you want tool reads to hit the database (or set **`NEO4J_READ_MODE=native`** in `.env` if you rely on env-only mode elsewhere).
- **`NEO4J_READ_MODE=llm_cypher`** (third compare column / Streamlit label **Neo4j (tool-free Cypher planner)**): the investigation LLM does **not** use named graph tools or provider tool APIs. It runs a multi-turn chat, emitting strict JSON with read-only Cypher per turn (`src/llm/cypher_planner.py`); queries are validated and executed on Aura. Judge and synthesis still consume the same chronological trace shape (steps use synthetic tool id **`__cypher__`**). Tool preflight and extension authoring are skipped in this mode.
- **``Unable to retrieve routing information``:** often fixed by using **direct Bolt** instead of the routing driver. **Aura** URIs (host ends with **`.databases.neo4j.io`**) now default to **`bolt+s://`** automatically when ``NEO4J_DRIVER_MODE`` is unset. To force the old routing driver, set **`NEO4J_DRIVER_MODE=routing`**. To force direct on any host, set **`NEO4J_DRIVER_MODE=direct`**. Restart Streamlit after changing driver settings (the driver is cached per process).
- **SSL / ``SSLCertVerificationError`` / ``Operation not permitted``:** the driver now sets **`SSL_CERT_FILE`** from **certifi** by default before connecting (disable with **`NEO4J_USE_CERTIFI_CA_BUNDLE=0`**). Still failing: **`pip install --upgrade certifi`** and macOS **`Install Certificates.command`**. **PoC-only:** **`NEO4J_TLS_INSECURE=1`** → **`bolt+ssc://`** (trust-all — **MITM risk**). If errors persist, try another network or a normal terminal (not a restricted sandbox).
...
**Ollama JSON answers:** Judge and synthesis use **compact prompts** (no full domain-doc block), **truncated traces** (see **`OLLAMA_MAX_TRACE_CHARS`** in `env.template`), and Ollama **`format=json`** for parseable output. If the model still returns an empty JSON `answer`, the app runs a **short plain-text synthesis fallback** so you still get prose.
...
```text
User question
        → Session context resolver (rewrite / clarify / pass-through; in-memory turns only)
        → Pre-run entity resolution (ambiguous names → picker; graph ids anchored)
...
**Investigations (hosted or local LLM):** create a **`.env`** file in the project root. Copy **`env.template`** to **`.env`** and fill in values (see comments in the template). Do not commit **`.env`**. At minimum, set keys for your chosen backend—for example:
...
**Session memory regression** (no LLM required for most cases):
